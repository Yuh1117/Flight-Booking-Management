...
def show_routes():
    ...
    return render_template(
        ...
        kw_depart_airport=kw_depart_airport if kw_depart_airport else "",
        kw_arrive_airport=kw_arrive_airport if kw_arrive_airport else "",
        airports=airports,
...
def get_airlines():
    airlines = dao.get_airlines()
    airlines_data = [{"id": airline.id, "name": airline.name} for airline in airlines]
    return jsonify(airlines_data)


@flights_bp.route("/api/seatclasses", methods=["GET"])
def get_seatclasses():
    seatclasses = dao.get_seat_classes()
    seatclasses_data = [
        {"id": seatclass.id, "name": seatclass.name} for seatclass in seatclasses
    ]
    return jsonify(seatclasses_data)
